// ===== BATE-PAPO AVANÇADO - FIREBASE WEB V8 - v516 =====
...
function debug(){if($("debugText"))$("debugText").textContent="Projeto: "+firebaseConfig.projectId+" | v515"}debug();
...
if("serviceWorker" in navigator) navigator.serviceWorker.register("service-worker.js?v=516").catch(console.warn);
...
try { if ('serviceWorker' in navigator) navigator.serviceWorker.register('service-worker.js?v=516').catch(console.warn); } catch(e) {}
...
// ===== v513 - Push FCM com chave VAPID informada =====
try { if ($('debugText')) $('debugText').textContent = 'Projeto: ' + firebaseConfig.projectId + ' | v513 FCM'; } catch(e) {}
try { if ('serviceWorker' in navigator) navigator.serviceWorker.register('service-worker.js?v=516').catch(console.warn); } catch(e) {}
...
// Chave VAPID já está em window.FCM_VAPID_KEY_V512.
...
async function initPushV512(){
  try {
    ...
    const reg = await navigator.serviceWorker.register('service-worker.js?v=516');
...
/* ===== v514: VAPID completa + home compacta + conversas sem duplicar ===== */
try { window.APP_VERSION = '516'; if ($('debugText')) $('debugText').textContent = 'Projeto: ferramentas-projeto | v516'; } catch(e) {}
...
async function renderChatsV514(filter=''){
  ...
  for (const c of items){
    ...
    const div = document.createElement('div');
    div.className = 'item chat-row-v514';
...
/* ===== v516: atualização limpa do PWA + push de chamada ===== */
try { window.APP_VERSION = '516'; if ($('debugText')) $('debugText').textContent = 'Projeto: ferramentas-projeto | v516'; } catch(e) {}

async function forceUpdatePwaV516(){
  try{
    if(!('serviceWorker' in navigator)) return;
    const reg = await navigator.serviceWorker.register('./service-worker.js?v=516', {updateViaCache:'none'});
    await reg.update();
    if(reg.waiting) reg.waiting.postMessage({type:'SKIP_WAITING'});
  }catch(e){ console.warn('SW update v516', e); }
}
forceUpdatePwaV516();

async function initPushV516(){
  try{
    if(!currentUser || !('Notification' in window) || !('serviceWorker' in navigator) || !firebase.messaging) return;
    const permission = await Notification.requestPermission();
    if(permission !== 'granted') return;
    ...
    const token = await messaging.getToken({vapidKey: window.FCM_VAPID_KEY_V512, serviceWorkerRegistration: reg});
    if(!token) return;
    await db.ref('usuarios/'+currentUser.uid+'/fcmTokens/'+safeKeyV512(token)).set({token, plataforma:'web-pwa', atualizado:Date.now()});
    await db.ref('usuarios/'+currentUser.uid).update({fcmToken:token, pushAtivo:true, appVersion:'516'});
    messaging.onMessage(payload=>{
      const n=payload.notification||{}, d=payload.data||{};
      if(d.type==='call') notifyV511(n.title||'Ligação recebida', n.body||'Alguém está ligando para você');
      else notifyV511(n.title||'Bate-papo', n.body||'Nova mensagem');
    });
  }catch(e){ console.warn('Erro initPushV516', e); }
}

const startAppV516_original = startApp;
startApp = function(){
  try { startAppV516_original(); } catch(e){ console.warn(e); }
  setTimeout(()=>{ try { initPushV516(); forceUpdatePwaV516(); } catch(e){ console.warn(e); } }, 900);
};

const startCallV516_original = startCall;
startCall = async function(type){
  const callType = type || 'audio';
  if(currentChatUser && currentUser){
    const title = callType === 'video' ? 'Chamada de vídeo' : 'Chamada de áudio';
    const body = (currentUser.email || 'Alguém') + ' está ligando para você';
    await queuePushV512(currentChatUser.uid, 'call', title, body, {type:'call', callType, fromUid:currentUser.uid, fromName:currentUser.email||'', url:'./index.html?v=516'});
  }
  return startCallV516_original(callType);
};
if (callEl('btnAudioCall')) callEl('btnAudioCall').onclick = () => startCall('audio');
if (callEl('btnVideoCall')) callEl('btnVideoCall').onclick = () => startCall('video');
if (callEl('infoCall')) callEl('infoCall').onclick = () => startCall('audio');
...
try { if ($('btnNotify')) $('btnNotify').onclick = () => initPushV516().then(()=>toast('Notificações ativadas.')); } catch(e) {}
