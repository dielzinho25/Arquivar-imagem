...
const CACHE_NAME = 'batepapo-cache-v514';
...
if (messaging) {
  messaging.onBackgroundMessage(payload => {
    ...
    self.registration.showNotification(n.title || 'Bate-papo', {
      body: n.body || 'Nova atualização',
      icon: 'icon-192.png',
      badge: 'icon-192.png',
      vibrate: d.type === 'call' ? [300,120,300,120,300,120,300] : [200,100,200],
      tag: d.type === 'call' ? 'batepapo-chamada' : 'batepapo-msg',
...
self.addEventListener('notificationclick', event => {
  ...
  event.waitUntil(clients.matchAll({type:'window', includeUncontrolled:true}).then(list => {
    ...
    if (clients.openWindow) return clients.openWindow('./?v=514');
  }));
});
